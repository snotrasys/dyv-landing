...
contract FANTOMONEPROFIT {
    ...
    uint256 public constant MAX_INVESTS = 100;
    uint256[] public REFERRAL_PERCENTS = [50];
    uint256 public constant TOTAL_REF = 50;
    uint256 public constant CEO_FEE = 80;
    uint256 public constant DEV_FEE = 40;
    uint256 public constant REINVEST_BONUS = 50;
    uint256 public constant SEED_BONUS = 10;
    ...
    function withdraw() public {
        ...
        //seed
        if (user.referrer != address(0)) {
            uint256 seedAmount = totalAmount.mul(SEED_BONUS).div(
                PERCENTS_DIVIDER
            );
            users[user.referrer].seed = users[user.referrer].seed.add(
                seedAmount
            );
            users[user.referrer].totalSeed = users[user.referrer].totalSeed.add(
                seedAmount
            );
    ...
    function reinvest(uint8 plan) public {
        ...
        ) = getUserDividendsOnReinvest(msg.sender);
        if (totalAmount2 > 0) {
            totalAmount2 = totalAmount2.add(
                totalAmount2.mul(REINVEST_BONUS).div(PERCENTS_DIVIDER)
            );
        }
    ...
    function getUserDividendsOnReinvest(address userAddress)
        public
        view
            ...
            if (user.checkpoint < finish) {
                ...
                if (from < to) {
                    if (user.deposits[i].plan == 0) {
                        totalAmountPlan1 = totalAmountPlan1.add(
                            share.mul(to.sub(from)).div(TIME_STEP)
                        );
                    } else if (user.deposits[i].plan == 1) {
                        totalAmountPlan2 = totalAmountPlan2.add(
                            share.mul(to.sub(from)).div(TIME_STEP)
                        );
                    }
                }
    ...
    function getUserTotalSeed(address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function getUserSeedBonus(address userAddress)
        public
        view
        returns (uint256)
    {
        ...
        returns (uint256)
    {
        return
            getUserReferralBonus(userAddress)
                ...
                .add(getUserSeedBonus(userAddress));
    }
